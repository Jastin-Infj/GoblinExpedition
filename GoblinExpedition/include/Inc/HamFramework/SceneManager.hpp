...
namespace ham
{
	...
	class SceneBase : s3d::Uncopyable
	{
	public:
		...
		virtual void drawFadeIn(double t) const
		{
			...
			s3d::Window::ClientRect().draw(m_manager->getFadeColor().setAlpha(1.0 - t));
		}
		...
		virtual void drawFadeOut(double t) const
		{
			...
			s3d::Window::ClientRect().draw(m_manager->getFadeColor().setAlpha(t));
		}
	...
	protected:
		...
		/// </returns>
		bool changeScene(const State& state, int transitionTimeMillisec = 1000, bool crossFade = false)
		{
	...
	private:
		...
		bool updateSingle()
		{
			s3d::int32 elapsed = m_stopwatch.ms();
		...
		bool updateCross()
		{
			const s3d::int32 elapsed = m_stopwatch.ms();
		...
		template<class Type>
		s3d::String getSceneName() const
		{
			return s3d::Format(L'[', static_cast<int>(m_currentState), L']');
		}

		template<>
		s3d::String getSceneName<s3d::String>() const
		{
			return s3d::Format(L'[', m_currentState, L']');
		}

		void drawSceneName() const
		{
			s3d::PutText(getSceneName<State>()).at(s3d::Window::Center());
		}
	...
	public:
		...
		/// シーンの変更が可能でフェードイン・アウトが開始される場合 true, それ以外の場合は false
		/// </returns>
		bool changeScene(const State& state, int transitionTimeMillisec, bool crossFade)
		{
