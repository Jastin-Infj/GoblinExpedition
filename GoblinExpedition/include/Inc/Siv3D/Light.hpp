...
namespace s3d
{
	...
	struct ShadowLight
	{
		...
		constexpr ShadowLight(
			const Vec3& _position = Vec3(5, 25, -45),
			const Vec3& _lookat = Vec3(0,0,0),
			double _nearClip = 1.0,
			double _farClip = 1024.0,
			double _fovDegree = 45.0
		)
			: position(_position)
			, lookat(_lookat)
			, nearClip(_nearClip)
		...
		static ShadowLight None()
		{
			ShadowLight shadowLight;
			shadowLight.type = Type::None;
			return shadowLight;
		}
	};
}
