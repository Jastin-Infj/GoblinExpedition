...
{
	template <>
	struct hash<s3d::Float3>
	{
		size_t operator () (const s3d::Float3& keyVal) const
		{
			return std::_Bitwise_hash<s3d::Float3>()(keyVal);
		}
	};

	template <>
	struct hash<s3d::Vec3>
	{
		size_t operator () (const s3d::Vec3& keyVal) const
		{
			return std::_Bitwise_hash<s3d::Vec3>()(keyVal);
		}
	};
}
