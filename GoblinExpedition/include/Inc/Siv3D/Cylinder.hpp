...
#pragma warning(default:4201)
		...
		const Cylinder& drawForward(const ColorF& color = Palette::White) const;

		const Cylinder& drawForward(const Texture& texture, const ColorF& color = Palette::White) const;

		const Cylinder& drawShadow() const;
	};
}
