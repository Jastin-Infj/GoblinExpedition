...
#pragma warning(default:4201)
		...
		const Cone& drawForward(const ColorF& color = Palette::White) const;

		const Cone& drawForward(const Texture& texture, const ColorF& color = Palette::White) const;

		const Cone& drawShadow() const;
	};
}
