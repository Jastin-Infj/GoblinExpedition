...
{
	template <>
	struct hash<s3d::Float4>
	{
		size_t operator () (const s3d::Float4& keyVal) const
		{
			return std::_Bitwise_hash<s3d::Float4>()(keyVal);
		}
	};

	template <>
	struct hash<s3d::Vec4>
	{
		size_t operator () (const s3d::Vec4& keyVal) const
		{
			return std::_Bitwise_hash<s3d::Vec4>()(keyVal);
		}
	};
}
