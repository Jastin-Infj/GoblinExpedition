...
#pragma warning(default:4201)
		...
		const Box& drawShadow() const;

		const Box& drawForward(const ColorF& color = Palette::White) const;

		const Box& drawForward(const Texture& texture, const ColorF& color = Palette::White) const;
	};
}
