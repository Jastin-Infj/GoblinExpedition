...
#pragma warning(default:4201)
		...
		const Plane& drawForward(const ColorF& color = Palette::White) const;

		const Plane& drawForward(const Texture& texture, const ColorF& color = Palette::White) const;

		const Plane& drawShadow() const;
	};
}
