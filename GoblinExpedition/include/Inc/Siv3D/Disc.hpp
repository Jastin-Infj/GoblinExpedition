...
#pragma warning(default:4201)
		...
		const Disc& drawForward(const ColorF& color = Palette::White) const;

		const Disc& drawForward(const Texture& texture, const ColorF& color = Palette::White) const;

		const Disc& drawShadow() const;
	};
}
