...
namespace s3d
{
	...
	inline constexpr Point Clamp(const Point& v, const Point& min, const Point& max)
	{
		return{ s3d::Clamp(v.x, min.x, max.x), s3d::Clamp(v.y, min.y, max.y) };
	}

	inline constexpr Float2 Clamp(const Float2& v, const Float2& min, const Float2& max)
	{
		return{ s3d::Clamp(v.x, min.x, max.x), s3d::Clamp(v.y, min.y, max.y) };
	}

	inline constexpr Vec2 Clamp(const Vec2& v, const Vec2& min, const Vec2& max)
	{
		return{ s3d::Clamp(v.x, min.x, max.x), s3d::Clamp(v.y, min.y, max.y) };
	}

	inline constexpr Float3 Clamp(const Float3& v, const Float3& min, const Float3& max)
	{
		return{ s3d::Clamp(v.x, min.x, max.x), s3d::Clamp(v.y, min.y, max.y), s3d::Clamp(v.z, min.z, max.z) };
	}

	inline constexpr Vec3 Clamp(const Vec3& v, const Vec3& min, const Vec3& max)
	{
		return{ s3d::Clamp(v.x, min.x, max.x), s3d::Clamp(v.y, min.y, max.y), s3d::Clamp(v.z, min.z, max.z) };
	}

	inline constexpr Float4 Clamp(const Float4& v, const Float4& min, const Float4& max)
	{
		return{ s3d::Clamp(v.x, min.x, max.x), s3d::Clamp(v.y, min.y, max.y), s3d::Clamp(v.z, min.z, max.z) , s3d::Clamp(v.w, min.w, max.w) };
	}

	inline constexpr Vec4 Clamp(const Vec4& v, const Vec4& min, const Vec4& max)
	{
		return{ s3d::Clamp(v.x, min.x, max.x), s3d::Clamp(v.y, min.y, max.y), s3d::Clamp(v.z, min.z, max.z) , s3d::Clamp(v.w, min.w, max.w) };
	}
}
