...
#pragma warning(default:4201)
		...
		const Sphere& drawForward(const ColorF& color = Palette::White) const;

		const Sphere& drawForward(const Texture& texture, const ColorF& color = Palette::White) const;

		const Sphere& drawShadow() const;
	};
}